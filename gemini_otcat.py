    ...
    for i in range(0, len(windows)):
        ...
        if len(winlist) != 0 and winlist[0] != '':
            ...
            for string in winlist:
                ...
                vals[0] = time.Time(t0.to_value('s'), format='unix', scale='utc')

                duration = float(vals[1])
                if duration == -1.0:  # -1=forever
                    vals[1] = infinity
                else:
                    vals[1] = duration / 3600000. * u.h

                vals[2] = int(vals[2])
                if vals[2] == -1:  # -1=forever
                    vals[2] = 1000

                vals[3] = float(vals[3]) / 3600000. * u.h
                obs_wins.append({'start': vals[0], 'dur': vals[1], 'reps': vals[2], 'per': vals[3]})

            new_windows.append(obs_wins)
        else:
